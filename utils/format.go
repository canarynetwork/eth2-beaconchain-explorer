...
func FormatEth1BlockHash(hash []byte) template.HTML {
	...
	if !Config.Chain.Mainnet {
		return template.HTML(fmt.Sprintf("<a href=\"%[1]s/block/0x%[2]x\">0x%[2]x</a>%[3]s", Config.Frontend.Eth1ExplorerBase, hash, copyBtn))
	}
	return template.HTML(fmt.Sprintf("<a href=\"https://etherchain.org/block/%[1]x\">%[1]x</a>%[2]s", hash, copyBtn))
}
...
func FormatEth1TxHash(hash []byte) template.HTML {
	...
	if !Config.Chain.Mainnet {
		return template.HTML(fmt.Sprintf(`<i class="fas fa-male mr-2"></i><a style="font-family: 'Roboto Mono'" href=\"%s/tx/0x%x\">%v</a>%v`, Config.Frontend.Eth1ExplorerBase, hash, FormatHash(hash), copyBtn))
	}
